...
export default function Home() {
  ...
  const [familyStateCounts, setFamilyStateCounts] = useState<Array<{ playerId: string; displayName: string; initials: string; count: number }>>([]);
              ...
              <div className="bg-[#eef1f4] px-3 py-5 sm:px-6 sm:py-7">
                <div className="mx-auto max-w-6xl overflow-hidden rounded-2xl border border-slate-200 bg-white text-[#10254a] shadow-sm">
                  ...
                  <div className="p-4 sm:p-6">
                    ...
                    {passportView === 'states' && <div><div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><div className="text-[9px] font-black uppercase tracking-widest text-[#06284a]">States I've Been To</div><div className="text-xl font-black">Tap any state you've visited</div></div><div className="text-[9px] font-bold text-slate-500">★ = sports visit</div></div>{familyStateCounts.length>0&&<div className="mb-4"><div className="mb-2 text-[9px] font-black uppercase tracking-widest text-[#846f59]">Family State Count</div><div className="grid grid-cols-3 gap-2 sm:grid-cols-5">{familyStateCounts.map(person=><div key={person.playerId} className={`rounded-xl border p-2 text-center ${person.playerId===signedInPlayer?.id?'border-[#c69a2d] bg-[#fff4c9]':'border-slate-200 bg-slate-50'}`}><div className="text-xl font-black text-[#06284a]">{person.count}</div><div className="mt-0.5 truncate text-[8px] font-black uppercase text-[#846f59]">{person.displayName}</div></div>)}</div></div>}<div className="grid grid-cols-5 gap-1.5 sm:grid-cols-8 md:grid-cols-10">{US_STATES.map(code=>{const sports=passportVisitEntries.some(e=>e.state===code);const visited=visitedStates.includes(code);return <button key={code} title={STATE_NAMES[code]} onClick={()=>toggleVisitedState(code)} className={`relative aspect-[1.15] rounded-lg border text-[9px] font-black transition ${sports?'border-[#9c6c16] bg-[#e4c36b] text-[#33220d]':visited?'border-[#557492] bg-[#b9cbd9] text-[#102b49]':'border-[#cbb98f] bg-[#f7edda] text-[#9b8a68]'}`}>{code}{sports&&<span className="absolute right-0.5 top-0 text-[7px]">★</span>}</button>})}</div><div className="mt-4 rounded-lg border border-[#ccb582] bg-[#f8eccd] p-3 text-[9px] font-semibold">Regular travel counts too. Sports entries automatically mark their state, while you can tap any other state you've visited in general.</div></div>}
